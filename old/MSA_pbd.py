...
def MSATool(input, driver):
    elem = driver.find_element(By.LINK_TEXT, 'Search')
...
def MSA(fasfile, no_protein, file, MSAlink, compare_string):
    pbd_data = fas_to_list(fasfile)
    count = 0
    ...
    for i in range(0,len(pbd_data),2):
        string = pbd_data[i] + "\n" + pbd_data[i+1] +"\n"
        inputstring += string
        count+=1
...
def fas_to_list(file):
    with open(file) as f:
        list = f.read().split("\n")
        list.pop()      # list[len(list)-1] is empty
        return list 
...
def main(): 
    csvfile = "PoreDB.fas"
    no_protein = 3999
    MSAlink = "https://toolkit.tuebingen.mpg.de/tools/muscle"
    list = fas_to_list("PoreDB_long.fas")    
    compare_string = list[0] + "\n" + list[1] +"\n"
    MSA(csvfile, no_protein, "MSA_long_0", MSAlink, compare_string)
